package com.somnambulist.spark.sql.scala.spark.sql
...
object CreateDataFrameFromJsonFileTest {
  def main(args: Array[String]): Unit = {
    ...
    val df: DataFrame = spark.read.json("./data/com.jyong.scala.json")
    df.show(100)
//    df.select($"name").show()
...
//    val com.jyong.scala.com.jyong.scala.spark = SparkSession.builder().appName("test").master("local").getOrCreate()
//    val df :Dataset[Row] = com.jyong.scala.com.jyong.scala.spark.read.com.jyong.scala.json("./data/com.jyong.scala.json")
//    val df = frame
//    val rdd: RDD[Row] = df.rdd
//    rdd.map(line=>{
